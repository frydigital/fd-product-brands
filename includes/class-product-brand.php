...
class WooCommerce_Product_Brand
{
  public function __construct()
  {
    ...
    add_action('product_brand_edit_form_fields', array($this, 'edit_display_field'), 70);
...
?>
  ...
  ?>
    ...
    <tr class="form-field term-display-wrap">
      <th scope="row"><label for="product_brand_weight"><?php _e('Weight', 'text_domain'); ?></label></th>
      <td>
        <input type="number" step="0.01" name="product_brand_weight" id="product_brand_weight" value="<?php echo esc_attr($weight); ?>" />
        <p class="description"><?php _e('Set the display weight of the brand.', 'text_domain'); ?></p>
      </td>
    </tr>
...
<?php
  ...
  public function save_display_field($term_id)
  {
    ...
    if (!empty($_POST['product_brand_weight'])) {
      update_term_meta($term_id, 'product_brand_weight', $_POST['product_brand_weight']);
    }
  ...
  public function display_column_content($content, $column_name, $term_id)
  {
    ...
    } elseif ($column_name === 'product_brand_weight') {
      $weight = get_term_meta($term_id, 'product_brand_weight', true);
      $content = !empty($weight) ? esc_html($weight) : '';
    }
    return $content;
  }
